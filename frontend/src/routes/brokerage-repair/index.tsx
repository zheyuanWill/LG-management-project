...
interface RepairProject extends Project {
  handed_over?: boolean
}
...
export default function BrokerageRepairIndex() {
  ...
  const { data: projects, isLoading } = useApiGet<RepairProject[]>('/projects', {
    params: { type: 'brokerage_repair' },
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">修船经纪项目</h1>
          <p className="text-muted-foreground mt-1">管理修船经纪项目,跟踪报价、合同与移交</p>
        </div>
        <Button onClick={() => navigate({ to: '/projects/new', search: { type: 'brokerage_repair' } })} size="lg">
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          ...
          <p className="text-lg">暂无修船经纪项目</p>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project) => (
            <Card
              ...
              onClick={() => navigate({ to: `/brokerage-repair/${project.id}` })}
            >
              <CardContent className="p-5">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className={project.customer_name ? '' : 'text-muted-foreground/50'}>{project.customer_name || '未设置'}</span>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  {(project as RepairProject).handed_over && (
                    <div className="flex items-center gap-1 text-xs text-green-600 bg-green-50 dark:bg-green-900/20 rounded px-2 py-1 w-fit">

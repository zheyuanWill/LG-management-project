...
export default function ProjectForm({
  ...
  useEffect(() => {
    if (open) {
      setName(project?.ship_name || '')
      setImo(project?.imo || '')
      setCustomerId(project?.owner_id ? String(project.owner_id) : '')
      setPlannedEndDate(project?.planned_completion_date || '')
      setDescription(project?.remarks || '')
      setErrors({})
    }
  }, [open, project])
  ...
  const onSubmit = async () => {
    ...
    try {
      const payload = {
        ship_name: name.trim(),
        imo: imo || undefined,
        owner_id: customerId ? parseInt(customerId) : undefined,
        planned_completion_date: plannedEndDate || undefined,
